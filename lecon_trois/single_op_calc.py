...
while True:
    ...
    try:
        user_number_oper = int(input('Задайте количество чисел для обработки: '))
        while user_number_oper <= 1:
            user_number_oper = int(input('Задайте НЕНУЛЕВОЕ и НЕЕДИНИЧНОЕ количество чисел: '))
    except ValueError:
        ...
        try:
            ...
            print('Результат вычисления по заданным параметрам равен: ', result)

        plus_travail = input('Желаете выполнить новое вычисление? Введите да/нет: ')
