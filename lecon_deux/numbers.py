...
try:
    ...
    if user_number < 1 or user_number > 999:
        print(
            "Вы ввели число вне рабочего диапазона! Перезапустите программу для корректной работы."
        )

    else:
        user_number_cents = user_number // 100
        user_number_dix = (user_number - (user_number_cents * 100)) // 10
        user_number_un = user_number - (
            user_number_cents * 100 + user_number_dix * 10
        )
        ...
        if user_number_cents < user_number_dix < user_number_un:
            ...
            user_number_cents > user_number_dix > user_number_un
            or user_number_cents == 0
            and user_number_un == 0
            ...
            user_number_cents == user_number_dix
            or user_number_cents == user_number_un
            or user_number_dix == user_number_un
        ):
